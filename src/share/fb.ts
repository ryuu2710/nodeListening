...
import { FacebookBodyParams, FacebookFetchOptions, FacebookGraphQLRequestDTO, FacebookHeaders, VariablesDTO } from '../types';
import { EMPTY_STRING, FB_API_DEFAULT_ENDPOINT, FB_API_FRIENDLY_NAME_FIRST_LOWERCASE_KEY, FB_API_FRIENDLY_NAME_FIRST_UPPERCASE_KEY, FETCH_FB_API_REDIRECT_KEY, HTTP_POST_METHOD, CDP_GET_REQUEST_POST_DATA_KEY, CDP_REQUEST_WILL_BE_SENT_KEY, STRING_TYPE } from '#constants/index.js';
...
export function parseFacebookBody(
    rawBody: string
...
async function BuildHeaderDTO(recordHeaders: Record<string, string>, cookieValue: string): Promise<FacebookHeaders> {
...
export async function BuildBodyParamsConfig(content: string): Promise<FacebookBodyParams> {
...
export async function BuildFbRequestOptionsForCallApi(headersRaw: Record<string, string>, bodyRaw: string, cookieValue: string): Promise<FacebookFetchOptions> {
...
export function WaitNextGraphQL(client: CDPSession, targetFriendlyName: string): Promise<{ headers: Record<string, string>; bodyRaw: string }> {
    return new Promise((resolve) => {
      const listener = async (event: any) => {
        const { requestId, request } = event;

        if (typeof request.url === STRING_TYPE && request.url.includes(FB_API_DEFAULT_ENDPOINT) && request.method === HTTP_POST_METHOD) {
          const allHeaders = request.headers as Record<string, string>;
          const friendlyName = allHeaders[FB_API_FRIENDLY_NAME_FIRST_UPPERCASE_KEY] || allHeaders[FB_API_FRIENDLY_NAME_FIRST_LOWERCASE_KEY] || EMPTY_STRING;
          ...
          if (friendlyName === targetFriendlyName) {
            ...
            if (!rawBody) {
              try {
                const resp = await client.send(CDP_GET_REQUEST_POST_DATA_KEY, { requestId });
